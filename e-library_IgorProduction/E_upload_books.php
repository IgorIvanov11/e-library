...
// Определяем список доступных статусов в зависимости от статуса пользователя
if ($_SESSION["STATUS"] === "Сотрудник") {
    $allowed_statuses = array("На экспертизу");
} elseif ($_SESSION["STATUS"] === "Оператор") {
    $allowed_statuses = array("Открытый", "Действующий", "Архивный");
} elseif ($_SESSION["STATUS"] === "Модератор") {
    $allowed_statuses = array("Открытый", "Действующий", "Архивный", "ДСП");
} else {
    // Для всех остальных пользователей статусы не ограничены
    $allowed_statuses = array("Открытый", "Действующий", "Архивный", "Скрытый", "Закрытый", "На экспертизу");
}
...
if (isset($_POST["submit"])) {
    ...
    // Проверяем, входит ли выбранный статус в список доступных статусов для текущего пользователя
    if (in_array($status, $allowed_statuses)) {
        ...
        echo "<p class='error'>Выбранный статус не доступен для вашего статуса пользователя</p>";
    }
}
?>
<!DOCTYPE HTML>
    ...
    <div id="wrapper">
        ...
        <div id="center">
            <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                ...
                <label>Статус</label> <!-- Добавлено поле для выбора статуса -->
                <select name="status">
                    <?php
                    foreach ($allowed_statuses as $allowed_status) {
                        echo "<option value=\"$allowed_status\">$allowed_status</option>";
                    }
                    ?>
                </select>
                <label>Описание</label> <!-- Добавлено поле для описания книги -->
